...
print(
    ...
    f"{validation_df['segment_id'].nunique():,}"
)
...
if not metadata_df["start_timestamp"].is_monotonic_increasing:
    raise ValueError(
        "Sequence start timestamps are not chronological."
    )

if (
    metadata_df["end_timestamp"]
    < metadata_df["start_timestamp"]
).any():
    raise ValueError(
        "Invalid sequence timestamp range detected."
    )

print("Sequence metadata validation passed.")

OUTPUT_PATH = (
    OUTPUT_DIR / "validation_sequence_metadata.parquet"
)
