...
def load_data():
    """
    Load the frozen Isolation Forest model, test sequences,
    ...
    if not THRESHOLD_PATH.exists():
        raise FileNotFoundError(
            f"Missing Isolation Forest threshold:\n{THRESHOLD_PATH}"
        )
...
def flatten_sequences(X):
    ...
    Isolation Forest operates on a two-dimensional feature
    matrix.
    """
...
def anomaly_scores(model, X):
    """
    Isolation Forest's score_samples() returns:

        larger = more normal

    Negating the values gives the standardized convention:
...
# ============================================================
# Align sequence-level artifacts
# ============================================================
...
def build_output(
    ...
    # --------------------------------------------------------
    # Ensure proxy labels are aligned by sequence_id rather
    # than relying blindly on row order.
    # --------------------------------------------------------
    ...
    #
    # IMPORTANT:
    # The threshold was selected previously using the
    # validation set. It is NOT recalibrated here.
    # --------------------------------------------------------
...
def main():
    ...
    print("=" * 70)
    print(
        "ISOLATION FOREST TEST SCORE GENERATION"
    )
    print("=" * 70)
